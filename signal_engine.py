...
LATE80_TRADES_HEADER = [
    "signal_timestamp",
    "target_window_start",
    "slug",
    "side",
    "ask_at_decision",
    "abs_delta_at_decision",
    "threshold_min_cents",
    "threshold_max_cents",
    "capital_before",
    "free_capital_before",
...
def late80_trades_path(coin: str, duration_minutes: int) -> str:
    return f"{coin}-{int(duration_minutes)}-trades-late_80.csv"
...
def apply_duration_paths(config: dict[str, Any], coin: str, duration_minutes: int) -> dict[str, Any]:
    ...
    if "late_80" in strategies and isinstance(strategies["late_80"], dict):
        l80 = dict(strategies["late_80"])
        override = l80.get("trades_file")
        if override and int(duration_minutes) == int(config.get("_config_duration_minutes", duration_minutes)):
            l80["trades_log_file"] = str(override)
        else:
            l80["trades_log_file"] = late80_trades_path(coin, duration_minutes)
        strategies["late_80"] = l80
...
def load_config(path: str = "config.yaml") -> dict[str, Any]:
    ...
    if "late_80" in strategies:
        l80 = strategies["late_80"]
        if not isinstance(l80, dict):
            raise ValueError("strategies.late_80 must be a mapping")
        miss = [k for k in LATE80_REQUIRED if k not in l80]
        if miss:
            raise ValueError(f"strategies.late_80 missing keys: {miss}")
        if l80["capital_mode"] not in ("locked", "unlocked"):
            raise ValueError("strategies.late_80.capital_mode must be 'locked' or 'unlocked'")
...
@dataclass
class Late80OpenTrade:
    signal_timestamp: int
    target_window_start: int
    slug: str
    side: str
    ask_at_decision: float
    abs_delta_at_decision: float
    threshold_min_cents: int
    threshold_max_cents: int
    capital_before: float
    free_capital_before: float
    locked_capital_before: float
    invested_amount: float
...
class Late80Simulator:
    def __init__(self, global_config: dict[str, Any], strategy_config: dict[str, Any]):
        ...
        cfg = strategy_config
        self.threshold_min_cents = int(cfg["threshold_min_cents"])
        self.threshold_max_cents = int(cfg["threshold_max_cents"])
        self.threshold_min = self.threshold_min_cents / 100.0
        self.threshold_max = self.threshold_max_cents / 100.0
        self.decision_remaining_seconds = float(cfg["decision_remaining_seconds"])
        self.use_delta_filter = bool(cfg["use_delta_filter"])
        self.min_abs_delta = float(cfg["min_abs_delta"])
        self.trades_log_file = str(
            cfg.get("trades_log_file")
            or late80_trades_path(self.coin, self.duration_minutes)
        )
        ...
        self.open_trades: dict[int, Late80OpenTrade] = {}
        self._decided_windows: set[int] = set()
        self.status_line = "l80: idle"
        self._ensure_trades_header()
    ...
    def _ensure_trades_header(self) -> None:
        ...
        if os.path.isfile(path) and os.path.getsize(path) > 0:
            with open(path, newline="", encoding="utf-8") as f:
                existing = next(csv.reader(f), None)
            if existing == LATE80_TRADES_HEADER:
                return
            archived = f"{path}.bak"
            os.replace(path, archived)
            print(f"⚠️  Late80 trades log schema updated; old file moved to {archived}")
        with open(path, mode="a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(LATE80_TRADES_HEADER)
    ...
    def _write_trades_rows(self, rows: list[dict[str, Any]]) -> None:
        ...
        tmp_path = f"{path}.tmp"
        with open(tmp_path, mode="w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=LATE80_TRADES_HEADER, extrasaction="ignore")
            writer.writeheader()
            for row in rows:
                writer.writerow({k: row.get(k, "") for k in LATE80_TRADES_HEADER})
        os.replace(tmp_path, path)
    ...
    def _update_pending_trade_row(self, target_window_start: int, row: dict[str, Any]) -> None:
        ...
        for i in range(len(rows) - 1, -1, -1):
            ...
            fill = str(existing.get("fill_type", "")).strip().lower()
            if fill in ("pending", ""):
                rows[i] = {k: row.get(k, "") for k in LATE80_TRADES_HEADER}
                updated = True
                break
        if not updated:
            rows.append({k: row.get(k, "") for k in LATE80_TRADES_HEADER})
        self._write_trades_rows(rows)
    ...
    def display_status(self, window_start: int | None = None) -> str:
        if window_start is not None and window_start in self.open_trades:
            trade = self.open_trades[window_start]
            return f"L80 {trade.side} {trade.contracts}c @{round(trade.entry * 100)}¢"
        if self.status_line:
            return self.status_line
        return "l80: idle"

    def _in_band(self, ask: float) -> bool:
        return self.threshold_min <= ask <= self.threshold_max
    ...
    def _log_skip(
        ...
        self._append_trades_row(
            {
                ...
                "slug": slug,
                "side": side,
                "ask_at_decision": ask if ask != "" else "",
                "abs_delta_at_decision": round(abs_delta, 6) if abs_delta else "",
                "threshold_min_cents": self.threshold_min_cents,
                "threshold_max_cents": self.threshold_max_cents,
                "capital_before": round(equity_before, 6),
                "free_capital_before": round(free_before, 6),
                "locked_capital_before": round(locked_before, 6),
                "invested_amount": 0,
        ...
        self.status_line = f"l80 skip | {reason}"
        self._print_event(f"➖ [late_80] skip | {reason} | {slug}")
    ...
    def _maybe_enter(
        ...
        slug = self._target_slug(window_start)
        free_before = self.capital.free_capital
        locked_before = self.capital.locked_capital
        equity_before = self.capital.equity
        abs_delta = abs(current_price - price_to_beat) if current_price > 0 and price_to_beat > 0 else 0.0

        up_ok = self._in_band(up_ask)
        down_ok = self._in_band(down_ask)
        if up_ok == down_ok:
            reason = "both_sides_qualify" if up_ok else "no_side_in_band"
            self._log_skip(
                window_start, slug, reason, abs_delta, free_before, locked_before, equity_before
            )
            return

        side = "Up" if up_ok else "Down"
        ask = up_ask if up_ok else down_ask
        ...
        if self.use_delta_filter:
            if current_price <= 0 or price_to_beat <= 0:
                self._log_skip(
                    window_start,
                    slug,
                    "missing_current_price_for_delta",
                    abs_delta,
                    free_before,
                    locked_before,
                    equity_before,
                ...
                return
            if abs_delta < self.min_abs_delta:
                self._log_skip(
                    window_start,
                    slug,
                    f"abs_delta={abs_delta:.4f}<{self.min_abs_delta}",
                    abs_delta,
                    free_before,
                    locked_before,
                    equity_before,
        ...
        contracts = self.capital.calculate_contracts_single(ask)
        required_cost = self.capital.required_cost_single(contracts, ask)
        if contracts <= 0 or required_cost <= 0:
            self._log_skip(
                window_start,
                slug,
        ...
        ok, reason = self.capital.try_lock(window_start, required_cost)
        if not ok:
            self._log_skip(
                window_start,
                slug,
        ...
        trade = Late80OpenTrade(
            signal_timestamp=int(time.time()),
            target_window_start=window_start,
            slug=slug,
            side=side,
            ask_at_decision=ask,
            abs_delta_at_decision=round(abs_delta, 6),
            threshold_min_cents=self.threshold_min_cents,
            threshold_max_cents=self.threshold_max_cents,
            capital_before=round(equity_before, 6),
            free_capital_before=round(free_before, 6),
        ...
        self._append_trades_row(
            {
                ...
                "slug": trade.slug,
                "side": trade.side,
                "ask_at_decision": trade.ask_at_decision,
                "abs_delta_at_decision": trade.abs_delta_at_decision,
                "threshold_min_cents": trade.threshold_min_cents,
                "threshold_max_cents": trade.threshold_max_cents,
                "capital_before": trade.capital_before,
                "free_capital_before": trade.free_capital_before,
                "locked_capital_before": trade.locked_capital_before,
                "invested_amount": trade.invested_amount,
        ...
        self.status_line = f"l80 {side} @{round(ask * 100)}¢"
        self._print_event(
            f"✅ [late_80] BUY {side} | ask={ask:.4f} | abs_delta={abs_delta:.4f} "
            f"| {contracts}c (${required_cost:.2f}) | {slug} "
            f"| free=${self.capital.free_capital:.2f}"
        )
    ...
    def _settle_open_trade(self, window_start: int, outcome: str) -> None:
        ...
        if outcome in ("Up", "Down"):
            ...
            pnl = trade.contracts * (exit_val - trade.entry)
            fill_type = "filled"
        else:
            exit_val = ""
            pnl = 0.0
            fill_type = "filled;outcome_unknown"

        _locked, free_after, equity_after = self.capital.release(window_start, pnl)
        note = "settled"
        if outcome not in ("Up", "Down"):
            note = "settled;outcome_unknown"
        ...
        self.status_line = f"l80 settled pnl={pnl:+.2f}"
        self._print_event(
            f"📒 [late_80] SETTLED {trade.slug} | side={trade.side} | outcome={outcome} "
            f"| pnl={pnl:+.4f} | equity=${equity_after:.2f}"
        )
        del self.open_trades[window_start]
    ...
    def on_window_update(
        ...
        if remaining_seconds <= self.decision_remaining_seconds:
            self._maybe_enter(window_start, price_to_beat, current_price, up_ask, down_ask)
        return None
...
class StrategyRunner:
    ...
    def from_config(cls, config: dict[str, Any]) -> StrategyRunner:
        ...
        l80_cfg = strategies_cfg.get("late_80")
        if isinstance(l80_cfg, dict) and l80_cfg.get("enabled"):
            enabled.append(Late80Simulator(config, l80_cfg))
            max_decision = max(max_decision, float(l80_cfg.get("decision_remaining_seconds", 0)))
    ...
    def summarize(self) -> str:
        ...
        for s in self.strategies:
            ...
            if isinstance(s, DualHedgeSimulator):
                name = "dual_hedge"
                lines.append(
                    f"  {name}: capital=${s.capital.equity:.2f} limit={s.limit_cents}¢ "
                    f"trades={s.trades_log_file} history={len(s.history)}"
                )
            elif isinstance(s, Late80Simulator):
                name = "late_80"
                lines.append(
                    f"  {name}: capital=${s.capital.equity:.2f} "
                    f"band={s.threshold_min_cents}-{s.threshold_max_cents}¢ "
