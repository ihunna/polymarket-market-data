...
import requests
import websocket
from polymarket_poller import fetch_polymarket_data, get_market_metadata_for_slug
...
def log_to_csv(timestamp, price_to_beat, final_coingecko_price, lowest_up, lowest_down, outcome):
    """Appends window results including price-to-beat, final CoinGecko price, and extremes to CSV."""
    ...
    with open(filename, mode="a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["Time stamp", "price_to_beat", "final_coingecko_price", "lowest_up", "lowest_down", "outcome"])
        writer.writerow([timestamp, price_to_beat, final_coingecko_price, lowest_up, lowest_down, outcome])

def fetch_coingecko_price():
    """Queries CoinGecko simple price REST API for the final settlement Solana price."""
    try:
...
def run_high_frequency_loop(ws_manager, price_to_beat):
    """Executes a 15-minute window loop tracking token asks and querying CoinGecko only at the buzzer."""
    now_utc = calendar.timegm(time.gmtime())
    window_start = (now_utc // WINDOW_DURATION_SECONDS) * WINDOW_DURATION_SECONDS
    ...
    while True:
        ...
        if current_time >= window_end:
            print("\n⏳ Window ended. Fetching final price from CoinGecko...")
            final_coingecko_price = fetch_coingecko_price()
            if final_coingecko_price == 0.0:
                print("⚠️ CoinGecko fetch failed, falling back to previous price-to-beat.")
                final_coingecko_price = price_to_beat
            ...
            log_to_csv(window_start, price_to_beat, final_coingecko_price, l_up_fmt, l_down_fmt, outcome)
            
            print(f"🏁 Window completed! Outcome: {outcome} | PTB: ${price_to_beat:.2f} | Final CoinGecko Price: ${final_coingecko_price:.2f} | Logged")
            
            return final_coingecko_price
